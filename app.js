...
const JOURNAL_STOP_WORDS = new Set([
  "about","after","again","agent","before","being","became","because","begins","between","choosing","compared","could","darker","decision","did","does","first","from","gathered","moon","journal","language","memory","notebook","pattern","river","signs","still","that","the","their","there","this","through","today","water","with","without","yesterday","maya","pollock","rembrandt","nereid","halimede","laomedeia","psamathe","neso","triton","proteus","larissa","hippocamp"
]);
const JOURNAL_CYCLE_DAYS = 3;
const LEXICON_MODES = { current: "current", display: "display", light: "light" };
const LEXICON_INFLUENCE_SCALE = 0.0225; // Lexicon correction: 50% weaker so Light Influence stays a whisper.
const LEXICON_BOOST_LIMIT = 0.18; // Lexicon correction: lower cap prevents semantic nudges from overpowering Base Q.
const LEXICON_ACTIVE_CONDITIONS = ["drought", "riverNear", "lowHealth", "lowThirst", "lowHunger", "lowSleep", "lowSocial"];
const LEXICON_STOP_WORDS = new Set([
  // General grammar / journal filler.
  "about","after","again","agent","before","being","became","because","begins","between","choosing","compared","could","darker","decision","does","first","from","moon","journal","notebook","signs","still","that","their","there","this","through","today","with","without","yesterday",
  // Agent / moon names.
  "maya","pollock","rembrandt","nereid","halimede","laomedeia","psamathe","neso","triton","proteus","larissa","hippocamp",
  // Repeated sentence-template words from the journal. These are blocked so Agent Lexicon learns concepts, not phrasing loops.
  "gathered","result","route","starting","separate","noise","forced","answered","near","watched","changed","paths","found","better","experiment","repeating"
]);

const LEXICON_CONCEPT_WEIGHTS = {
  drought: 1.9,
  river: 1.65,
  water: 1.55,
  pain: 1.75,
...
function lexiconConceptWeight(word){
  return Number(LEXICON_CONCEPT_WEIGHTS[word]) || 1;
}

const state = { day: 1, running: false, started: false, timer: null, islands: [], dailyLog: [], rollingLog: [], lastJournalRenderDay: 0, bountifulHarvestDaysRemaining: 0, nextBountifulHarvestDay: 1, lexiconMode: LEXICON_MODES.current };
...
const els = {
  ...
  lexiconModeSelect: document.getElementById("lexiconModeSelect"),
  lexiconReadout: document.getElementById("lexiconReadout")
};
...
function makeAgent(template, island){
  ...
  return {
    ...
    daily: [], totalReward: 0, qShiftToday: 0, discoveries: 0, semanticBoostToday: makeQDrive(), lexicon: makeLexicon(), islandLetter: island.letter
  };
}
...
function resetSimulation(){
  ...
  for(const island of state.islands){
    for(const agent of island.agents){
      addJournal(agent, `${agent.name}: I open the first notebook page and listen to ${island.name} Moon before choosing.`);
    }
  }
  render(true);
}
...
function actionScores(island, agent, qKey){
  ...
  const semantic = semanticActionBoosts(island, agent);
  return {
    gather: q.gather + hungerNeed * 0.045 + thirstNeed * 0.038 + island.food * 0.012 + island.water * 0.01 + riverBonus + (agent.bias.gather || 0) + drive.gather + semantic.gather,
    move: q.move + (100 - agent.memory) * 0.018 + (island.drought ? 0.9 : 0) + (agent.bias.move || 0) + drive.move + semantic.move,
    rest: q.rest + (100 - agent.sleep) * 0.055 + (100 - agent.health) * 0.025 + (agent.bias.rest || 0) + drive.rest + semantic.rest,
    socialize: q.socialize + (100 - agent.social) * 0.06 + (agent.bias.socialize || 0) + drive.socialize + semantic.socialize,
    observe: q.observe + (100 - agent.confidence) * 0.035 + agent.memory * 0.012 + (agent.bias.observe || 0) + drive.observe + semantic.observe
  };
}


function currentLexiconMode(){
  return Object.values(LEXICON_MODES).includes(state.lexiconMode) ? state.lexiconMode : LEXICON_MODES.current;
}

function lexiconModeLabel(){
  if(currentLexiconMode() === LEXICON_MODES.light) return "Light Influence";
  if(currentLexiconMode() === LEXICON_MODES.display) return "Display Only";
  return "Current Model";
}

function lexiconConditionSnapshot(island, agent){
  return {
    drought: !!island.drought,
    riverNear: !!island.riverNear,
    lowHealth: agent.health < 35,
    lowThirst: agent.thirst < 35,
    lowHunger: agent.hunger < 35,
    lowSleep: agent.sleep < 35,
    lowSocial: agent.social < 35
  };
}

function tokenizeLexiconText(text){
  return String(text).toLowerCase().match(/[a-z][a-z-]{3,}/g)?.filter(word => !LEXICON_STOP_WORDS.has(word)) || [];
}

function learnLexiconFromText(island, agent, text, action, reward){
  if(currentLexiconMode() === LEXICON_MODES.current || !text || !ACTIONS.includes(action)) return;
  if(!agent.lexicon) agent.lexicon = makeLexicon();
  const words = [...new Set(tokenizeLexiconText(text))].slice(0, 8);
  if(!words.length) return;
  const conditions = lexiconConditionSnapshot(island, agent);
  for(const word of words){
    const weight = lexiconConceptWeight(word);
    const entry = agent.lexicon[word] || { count: 0, rewardTotal: 0, actions: makeQDrive(), conditions: {}, conceptWeight: weight };
    entry.conceptWeight = weight;
    entry.count += 1;
    entry.rewardTotal += reward * weight;
    entry.actions[action] = (Number(entry.actions[action]) || 0) + reward * weight;
    for(const key of LEXICON_ACTIVE_CONDITIONS){
      if(conditions[key]) entry.conditions[key] = (Number(entry.conditions[key]) || 0) + 1;
    }
    agent.lexicon[word] = entry;
  }
  const keys = Object.keys(agent.lexicon);
  if(keys.length > 42){
    keys.sort((a,b) => (agent.lexicon[a].count || 0) - (agent.lexicon[b].count || 0));
    for(const key of keys.slice(0, keys.length - 42)) delete agent.lexicon[key];
  }
}

function semanticActionBoosts(island, agent){
  const neutral = makeQDrive();
  agent.semanticBoostToday = neutral;
  if(currentLexiconMode() !== LEXICON_MODES.light || !agent.lexicon) return neutral;
  const conditions = lexiconConditionSnapshot(island, agent);
  const boosts = makeQDrive();
  for(const entry of Object.values(agent.lexicon)){
    if(!entry || !entry.count) continue;
    const conceptWeight = clamp(Number(entry.conceptWeight) || 1, 1, 2);
    let relevance = 0;
    for(const key of LEXICON_ACTIVE_CONDITIONS){
      if(conditions[key]) relevance += (Number(entry.conditions?.[key]) || 0) / Math.max(1, entry.count);
    }
    if(relevance <= 0) continue;
    for(const action of ACTIONS){
      const actionSignal = (Number(entry.actions?.[action]) || 0) / Math.max(1, entry.count);
      boosts[action] += relevance * actionSignal * LEXICON_INFLUENCE_SCALE * conceptWeight;
    }
    const rewardAvg = entry.rewardTotal / Math.max(1, entry.count);
    if(rewardAvg < -1 && (conditions.lowHealth || conditions.lowThirst || conditions.lowHunger)){
      boosts.observe += Math.abs(rewardAvg) * 0.012;
      boosts.rest += conditions.lowHealth ? Math.abs(rewardAvg) * 0.01 : 0;
      boosts.move -= Math.abs(rewardAvg) * 0.008;
    }
  }
  for(const action of ACTIONS) boosts[action] = round(clamp(boosts[action], -LEXICON_BOOST_LIMIT, LEXICON_BOOST_LIMIT), 3);
  agent.semanticBoostToday = boosts;
  return boosts;
}

function lexiconWordCount(agent){
  return Object.keys(agent.lexicon || {}).length;
}

function topLexiconWords(limit = 5){
  const totals = new Map();
  for(const island of state.islands){
    for(const agent of island.agents){
      for(const [word, entry] of Object.entries(agent.lexicon || {})){
        totals.set(word, (totals.get(word) || 0) + ((entry.count || 0) * lexiconConceptWeight(word)));
      }
    }
  }
  return [...totals.entries()].sort((a,b) => b[1] - a[1] || a[0].localeCompare(b[0])).slice(0, limit).map(([word, score]) => [word, Math.round(score)]);
}
...
function journalFromLearning(island, agent, reward, signal){
  ...
  if(agent.memory > 72 && agent.confidence > 58) fragments.push(`${agent.name}: The moon feels smaller because memory now predicts it.`);
...
function dailyRecord(island, agent, action, reward){
  ...
  return {
    ...
    alive: agent.alive !== false, respawnDaysRemaining: agent.respawnDaysRemaining || 0, lexiconMode: currentLexiconMode(), lexiconWords: lexiconWordCount(agent),
    semanticBoostGather: round((agent.semanticBoostToday?.gather || 0), 3), semanticBoostMove: round((agent.semanticBoostToday?.move || 0), 3), semanticBoostRest: round((agent.semanticBoostToday?.rest || 0), 3), semanticBoostSocialize: round((agent.semanticBoostToday?.socialize || 0), 3), semanticBoostObserve: round((agent.semanticBoostToday?.observe || 0), 3),
...
function simulateDay(){
  ...
  for(const island of state.islands){
    ...
    for(const agent of island.agents){
      agent.qShiftToday = 0;
      agent.semanticBoostToday = makeQDrive();
      ...
      learnLexiconFromText(island, agent, signal, decision.action, reward);
...
function render(forceJournal = false){
  ...
  const complete = state.day > MAX_DAYS;
  els.runState.textContent = state.running ? "RUNNING" : (complete ? "COMPLETE" : (state.started ? "PAUSED" : "READY"));
  if(els.startBtn){
    els.startBtn.textContent = state.started && !state.running && !complete ? "Resume" : "Start";
    els.startBtn.disabled = complete;
  }
  els.pauseBtn.textContent = state.running ? "Pause" : "Pause";
  els.pauseBtn.disabled = !state.started || complete || !state.running;
  if(els.journalPauseBtn){
    els.journalPauseBtn.textContent = state.running ? "Pause Feed" : (state.started ? "Resume Feed" : "Feed Ready");
    els.journalPauseBtn.disabled = !state.started || complete;
  }
  ...
  els.runSummary.innerHTML = `
    ...
    <span><b>Islands</b> 10</span>
    <span><b>Agents</b> 30</span>
    <span><b>Deaths</b> ${deaths}</span>
    <span><b>Respawn</b> ${respawning}</span>
    <span><b>${harvestStatus}</b></span>
    <span><b>Longest life</b> ${longestLife}d</span>
    <span><b>Confidence</b> ${avgConfidence}%</span>
  `;
...
function renderLexiconPanel(){
  if(els.lexiconModeSelect) els.lexiconModeSelect.value = currentLexiconMode();
  if(!els.lexiconReadout) return;
  const allAgents = state.islands.flatMap(island => island.agents);
  const totalWords = allAgents.reduce((total, agent) => total + lexiconWordCount(agent), 0);
  const activeSignals = allAgents.reduce((total, agent) => total + ACTIONS.reduce((sum, action) => sum + Math.abs(Number(agent.semanticBoostToday?.[action]) || 0), 0), 0);
  const topWords = topLexiconWords(3).map(([word, count]) => `<span>${escapeHtml(word)} <b>${count}</b></span>`).join("") || `<span>no lexicon words yet</span>`;
  els.lexiconReadout.innerHTML = `
    <div class="lexicon-chip"><span>Mode</span><b>${lexiconModeLabel()}</b></div>
    <div class="lexicon-chip"><span>Stored Words</span><b>${totalWords}</b></div>
    <div class="lexicon-chip"><span>Signal</span><b>${round(activeSignals, 2)}</b></div>
    <div class="lexicon-words"><span>Top Words</span><div>${topWords}</div></div>
  `;
}

function updateLexiconMode(value){
  state.lexiconMode = Object.values(LEXICON_MODES).includes(value) ? value : LEXICON_MODES.current;
  render(true);
}
...
function downloadJSON(){
  const payload = {
    runId: RUN_ID, simulator: "Microsmos", maxDays: MAX_DAYS, lexiconMode: currentLexiconMode(),
...
function toggleRunning(){
  if(!state.started || !state.running) startRun();
  else pauseRun();
}
...
if(els.lexiconModeSelect){
  els.lexiconModeSelect.addEventListener("change", event => updateLexiconMode(event.target.value));
}
