...
const MEMORY_DECAY_RATE = 0.70; // Balance pass: slower memory decay; memory loss is now 1.12/day instead of 1.28/day.
const RESOURCE_REGEN_MULTIPLIER = 1.265; // Balance pass: +15% water/general resource regeneration from the previous 1.10 setting.
const RESOURCE_CAP = 200; // Balance pass: doubled island food/water storage cap from 100 to 200.
const FOOD_REGEN_MULTIPLIER = RESOURCE_REGEN_MULTIPLIER * 2; // Balance pass: food regeneration doubled while water regen stays at RESOURCE_REGEN_MULTIPLIER.
...
function performAction(island, agent, action, explored){
  ...
  if(action === "socialize"){
    ...
    reward += 1.449 + clueGain * 0.1512; // Balance pass: +5% social reward from the +20% social test setting.
...
function downloadJSON(){
  const payload = {
    ...
    tuning: { hungerThirstDriveMultiplier: HUNGER_THIRST_DRIVE_MULTIPLIER, memoryDecayRate: MEMORY_DECAY_RATE, resourceRegenMultiplier: RESOURCE_REGEN_MULTIPLIER, foodRegenMultiplier: FOOD_REGEN_MULTIPLIER, resourceCap: RESOURCE_CAP, riverWaterBenefit: RIVER_WATER_BENEFIT, droughtRiverBuff: DROUGHT_RIVER_BUFF, tickMs: TICK_MS, journalRenderEveryDays: JOURNAL_RENDER_EVERY_DAYS },
