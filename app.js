...
const MEMORY_DECAY_RATE = 0.80; // 20% slower memory degradation.
const RESOURCE_REGEN_MULTIPLIER = 1.10; // 10% increased food/water regeneration.
...
const state = { day: 1, running: true, timer: null, islands: [], dailyLog: [], rollingLog: [], lastJournalRenderDay: 0 };
...
function makeAgent(template, island){
  ...
  return {
    ...
    health: 100, hunger: 82, thirst: 82, sleep: 80, social: 70 + Math.random() * 18,
    memory: 32 + Math.random() * 20, confidence: 4, rewardStreak: 0, deaths: 0,
    birthDay: state.day, longestLife: 0, lastLifeDelta: 0,
    actionCounts: Object.fromEntries(ACTIONS.map(action => [action, 0])), q, lastAction: "observe", lastBest: "observe",
...
function actionScores(island, agent, qKey){
  ...
  return {
    gather: q.gather + hungerNeed * 0.045 + thirstNeed * 0.038 + island.food * 0.012 + island.water * 0.01 + riverBonus + (agent.bias.gather || 0),
    move: q.move + (100 - agent.memory) * 0.018 + (island.drought ? 0.9 : 0) + (agent.bias.move || 0),
    rest: q.rest + (100 - agent.sleep) * 0.055 + (100 - agent.health) * 0.025 + (agent.bias.rest || 0),
    socialize: q.socialize + (100 - agent.social) * 0.05 + (agent.bias.socialize || 0),
    observe: q.observe + (100 - agent.confidence) * 0.035 + agent.memory * 0.012 + (agent.bias.observe || 0)
  };
}
...
function decayAndRegenerateIsland(island){
  ...
  const droughtPenalty = island.drought ? 0.45 : 1;
  const riverRegenBuff = island.riverNear ? (island.drought ? DROUGHT_RIVER_BUFF : RIVER_WATER_BENEFIT) * island.riverAffinity : 1;
  island.food = clamp(island.food + island.fertility * RESOURCE_REGEN_MULTIPLIER * droughtPenalty, 0, 100);
  island.water = clamp(island.water + island.aquifer * RESOURCE_REGEN_MULTIPLIER * riverRegenBuff * droughtPenalty, 0, 100);
}
...
function performAction(island, agent, action, explored){
  ...
  if(action === "gather"){
    ...
    island.food = clamp(island.food - foodGain * 0.74, 0, 100);
    island.water = clamp(island.water - waterGain * 0.66, 0, 100);
  ...
  if(action === "move"){
    ...
    if(foundResource){
      island.food = clamp(island.food + 9, 0, 100);
      island.water = clamp(island.water + (island.riverNear ? 12 : 6), 0, 100);
      agent.discoveries += 1;
      reward += 2.6;
  ...
  if(action === "socialize"){
    ...
    peer.social = clamp(peer.social + 3, 0, 100);
    reward += 1.15 + clueGain * 0.12;
  ...
  if(agent.health <= 0){
    ...
    agent.deaths += 1;
    reward -= 8;
    addJournal(agent, `${agent.name}: The notebook stopped. A new copy begins with the old warning marks.`);
    agent.health = 84; agent.hunger = 70; agent.thirst = 70; agent.sleep = 76; agent.social = 62;
...
function render(forceJournal = false){
  ...
  els.runSummary.textContent = `Run ${RUN_ID} • islands 10 • agents 30 • deaths ${deaths} • longest life ${longestLife} days • average confidence ${avgConfidence}%`;
  els.logCount.textContent = `${state.dailyLog.length.toLocaleString()} daily agent records • ${state.rollingLog.length.toLocaleString()} rolling island records`;
    ...
    <article class="journal-card spotlight-card">
      ...
      <div class="spotlight-body">
        ...
        <div class="vocab-row" aria-label="Novel vocabulary">
          ...
          ${vocab.length ? vocab.map(word => `<span class="vocab-chip">${escapeHtml(word)}</span>`).join("") : `<span class="vocab-chip muted-chip">low novelty</span>`}
        </div>
      </div>
    </article>
    <div class="sample-rail" aria-label="Five sampled journal entries">
...
function downloadJSON(){
  const payload = {
    ...
    tuning: { hungerThirstDriveMultiplier: HUNGER_THIRST_DRIVE_MULTIPLIER, memoryDecayRate: MEMORY_DECAY_RATE, resourceRegenMultiplier: RESOURCE_REGEN_MULTIPLIER, riverWaterBenefit: RIVER_WATER_BENEFIT, droughtRiverBuff: DROUGHT_RIVER_BUFF, tickMs: TICK_MS, journalRenderEveryDays: JOURNAL_RENDER_EVERY_DAYS },
    islands: state.islands.map(island => ({
      ...
      agents: island.agents.map(agent => ({ name: agent.name, temperament: agent.temperament, deaths: agent.deaths, longestLife: Math.max(agent.longestLife || 0, currentLifeAge(agent, Math.min(state.day, MAX_DAYS))), currentLifeAge: currentLifeAge(agent, Math.min(state.day, MAX_DAYS)), lastLifeDelta: agent.lastLifeDelta || 0, discoveries: agent.discoveries, actionCounts: agent.actionCounts, journal: agent.journal, q: agent.q }))
    })),
