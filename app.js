...
const LEXICON_INFLUENCE_SCALE = 0.045;
const LEXICON_BOOST_LIMIT = 0.35;
...
const LEXICON_STOP_WORDS = new Set([
  "about","after","again","agent","before","being","became","because","begins","between","choosing","compared","could","darker","decision","does","first","from","island","journal","notebook","pattern","signs","still","that","their","there","this","through","today","with","without","yesterday","maya","pollock","rembrandt","nereid","halimede","laomedeia","psamathe","neso","triton","proteus","larissa","hippocamp"
]);
...
function learnLexiconFromText(island, agent, text, action, reward){
  ...
  for(const word of words){
    const entry = agent.lexicon[word] || { count: 0, rewardTotal: 0, actions: makeQDrive(), conditions: {} };
    entry.count += 1;
    entry.rewardTotal += reward;
    entry.actions[action] = (Number(entry.actions[action]) || 0) + reward;
...
function semanticActionBoosts(island, agent){
  ...
  for(const entry of Object.values(agent.lexicon)){
    ...
    for(const action of ACTIONS){
      ...
      boosts[action] += relevance * actionSignal * LEXICON_INFLUENCE_SCALE;
    }
  ...
  for(const island of state.islands){
    for(const agent of island.agents){
      for(const [word, entry] of Object.entries(agent.lexicon || {})){
        totals.set(word, (totals.get(word) || 0) + (entry.count || 0));
      }
    }
  }
...
function renderLexiconPanel(){
  ...
  const topWords = topLexiconWords(5).map(([word, count]) => `<span>${escapeHtml(word)} <b>${count}</b></span>`).join("") || `<span>no lexicon words yet</span>`;
